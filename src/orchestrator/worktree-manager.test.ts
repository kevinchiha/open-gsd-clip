...
describe('WorktreeManager', () => {
  ...
  describe('createWorktree', () => {
    it('creates worktreeBase directory with recursive:true', async () => {
      // branch --list returns empty (no existing branch)
      mockedExeca.mockResolvedValue({
        stdout: '',
        stderr: '',
        exitCode: 0,
    ...
    it('calls git worktree add -b with correct args and cwd', async () => {
      ...
      const worktreeAddCall = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
      ...
      expect(worktreeAddCall![2]).toEqual(
    ...
    it('checks for existing branch via git branch --list before creating', async () => {
      ...
      const branchListCall = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('branch') &&
          (call[1] as string[]).includes('--list'),
      );
      expect(branchListCall).toBeDefined();
      expect(branchListCall![1]).toEqual([
        'branch',
        '--list',
        'gsd/phase-5',
      ]);
    ...
    it('cleans up stale branch/worktree if branch already exists', async () => {
      // First call (branch --list) returns existing branch
      let callIndex = 0;
      mockedExeca.mockImplementation(async (..._args: unknown[]) => {
        callIndex++;
        if (callIndex === 1) {
          // git branch --list returns branch name (exists)
          return { stdout: '  gsd/phase-2', stderr: '', exitCode: 0 } as never;
        }
      ...
      // Should have called worktree remove and branch -D for cleanup
      const worktreeRemoveCall = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('worktree') &&
          (call[1] as string[]).includes('remove'),
      );
      const branchDeleteCall = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('branch') &&
  ...
  describe('removeWorktree', () => {
    it('calls git worktree remove then git branch -d', async () => {
      ...
      const worktreeRemove = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('worktree') &&
          (call[1] as string[]).includes('remove'),
      );
      const branchDelete = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('branch') &&
          (call[1] as string[]).includes('-d'),
  ...
  describe('pruneStaleWorktrees', () => {
    it('calls git worktree prune', async () => {
      ...
      const pruneCall = mockedExeca.mock.calls.find(
        (call) =>
          call[0] === 'git' &&
          (call[1] as string[]).includes('worktree') &&
          (call[1] as string[]).includes('prune'),
      );
